...
def create_parser() -> FlexibleArgumentParser:
    ...
    parser.set_defaults(max_num_batched_tokens=1024000)
    parser.set_defaults(gpu_memory_utilization=0.95)
    ...
    bench.add_argument("--min-seq-len", type=int, default=100)
...
def prepare_prompts(dataset: list[dict], batch_size: int,
                    seq_len: int, rng: random.Random) -> list[str]:
    prompts = []
...
def run_one_case(
    ...
    prompts = prepare_prompts(dataset, batch_size, seq_len, rng)
