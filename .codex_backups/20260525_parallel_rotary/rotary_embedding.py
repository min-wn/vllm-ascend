...
#
# Per-slot cos/sin buffers to support parallel split execution.
# Each parallel split thread uses a different slot_id to avoid
# concurrent inplace writes to the same tensor.
_NUM_COS_SIN_SLOTS = 2  # max parallel splits supported
_cos_mla: Optional[torch.Tensor] = None
_sin_mla: Optional[torch.Tensor] = None
_cos_sin_cache: Optional[torch.Tensor] = None
_cos_slots: list[Optional[torch.Tensor]] = [None] * _NUM_COS_SIN_SLOTS
...
def set_cos_and_sin(vllm_config, max_num_reqs, decode_token_per_req, dtype,
    ...
    if _cos_mla is not None or \
        ...
        _cos_slots[0] is not None or \
        _sin_slots[0] is not None:
        return
    ...
    if model_config.use_mla:
        ...
        for i in range(_NUM_COS_SIN_SLOTS):
            _cos_slots[i] = torch.ones(1,
                              max_num_batched_tokens,
                              1,
                              rope_dim,
                              dtype=dtype,
                              device=device)
            _sin_slots[i] = torch.zeros(1,
                               max_num_batched_tokens,
                               1,
                               rope_dim,
                               dtype=dtype,
...
def update_cos_sin(positions, slot_id=0):
    ...
    if _cos_sin_cache is None or \
        _cos_slots[slot_id] is None or \
        _sin_slots[slot_id] is None:
        return
