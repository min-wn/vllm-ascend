...
from vllm_ascend.worker.ubatch_utils import ubatch_split, split_batch_split, SplitBatchSlices
...
class NPUModelRunner(GPUModelRunner):
    ...
    def _generate_process_reqs_hidden_states(self, maybe_padded_num_tokens,
        ...
        if (forward_context.cudagraph_runtime_mode == CUDAGraphMode.FULL
                ...
                and not isinstance(self.model, AscendSplitBatchWrapper)
                ):
    ...
    def _update_attn_params_for_wrapper(self, forward_context, num_tokens):
        ...
        - AscendSplitBatchWrapper: Skip update here, will be done inside wrapper
    ...
    def _update_single_mla_attn_params(self, attn_metadata_dict, num_tokens):
        """Update MLA attention parameters for a single attn_metadata dict."""
        # Create a minimal context with the single attn_metadata
        single_context = type('ForwardContext', (), {
            'attn_metadata': attn_metadata_dict,
            'is_mtp_model': get_forward_context().is_mtp_model,
            'capturing': get_forward_context().capturing,
            'is_mla_model': get_forward_context().is_mla_model,
        })()
        update_mla_attn_params(
            self.update_stream, single_context, num_tokens,
            self.vllm_config.speculative_config)
    
    def _update_single_attn_params(self, attn_metadata_dict, num_tokens):
        """Update attention parameters for a single attn_metadata dict."""
        # Create a minimal context with the single attn_metadata
        single_context = type('ForwardContext', (), {
            'attn_metadata': attn_metadata_dict,
            'is_mtp_model': get_forward_context().is_mtp_model,
            'capturing': get_forward_context().capturing,
            'is_mla_model': getattr(get_forward_context(), 'is_mla_model', False),
        })()
        update_attn_params(
            self.update_stream, single_context, num_tokens,
            self.vllm_config)
    ...
    def load_model(self) -> None:
        ...
        # - Split-batch: AscendSplitBatchWrapper
        ...
        if self.parallel_config.enable_dbo:
            ...
            if self.compilation_config.cudagraph_mode.has_full_cudagraphs():
                self.model = AscendSplitBatchWrapper(self.model,
                                                     self.vllm_config,
                                                     CUDAGraphMode.FULL,
                                                     self.device)
            else:
